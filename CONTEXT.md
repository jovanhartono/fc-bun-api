...
**Complaint**:
A Customer's post-pickup grievance about an `OrderService` — the pair was cleaned poorly, they are dissatisfied. A first-class, **append-only** record: `order_service_id` FK → the original (complained) line, `status` (`open`/`closed`), `resolution` (`rework`/`refund`/`rejected`, set on close), free-text reason, `voucher_promised`, `opened_by`/`closed_by`, timestamps. Owns **0..N** Rework lines. Handling is an **escalation ladder** — re-clean first (a Rework); if still unsatisfactory, refund the original line and promise a goodwill voucher. Opening, adding a rework, and resolving are open to **any staff** (the cashier takes the return); escalating to a refund is **admin-only** (the existing money gate). At most **one open Complaint per line** (partial unique index); a closed Complaint must record a `resolution` (CHECK). `picked_up` stays terminal throughout — a Complaint never mutates the original line. **Complaint rate** (Complaints ÷ OrderService lines, ~1–2%) is a tracked metric on its own Complaints menu/page (the Reports rate panel is a deferred follow-up). The goodwill **voucher is out of band** — recorded as *promised* only; staff issue it manually (no customer-scoped Campaign in v1). The ~2-day "complain by" window is verbal, **not** system-enforced. See [ADR-0013](docs/adr/0013-complaint-and-rework-line.md).
